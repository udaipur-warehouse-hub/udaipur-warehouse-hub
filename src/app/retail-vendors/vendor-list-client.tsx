...
import type { RetailVendor } from "@/lib/types";
import { Panel } from "@/components/panel";

export function VendorListClient() {
  const [vendors, setVendors] = useState<RetailVendor[]>([]);
  ...
  async function load(query = "") {
    setLoading(true);
    const res = await fetch(`/api/retail-vendors?q=${encodeURIComponent(query)}`);
...
function AddVendorModal({ onClose, onAdded }: { onClose: () => void; onAdded: (v: RetailVendor) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-20 bg-black/30 flex items-center sm:items-center justify-center p-4" onClick={onClose}>
      <div className="bg-surface rounded-2xl p-5 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
        <h2 className="font-semibold mb-3">Add a vendor</h2>
        {error && (
