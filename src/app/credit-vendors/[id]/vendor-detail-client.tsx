...
import type { CreditVendor, VendorLedgerEntry } from "@/lib/types";
import { Panel } from "@/components/panel";

export function VendorDetailClient({ vendorId }: { vendorId: string }) {
  const [vendor, setVendor] = useState<CreditVendor | null>(null);
  ...
  async function load() {
    setLoading(true);
    const res = await fetch(`/api/credit-vendors/${vendorId}`);
  ...
  return (
    <div className="space-y-5">
      <Panel
        title={vendor.name}
        subtitle={[vendor.phone, vendor.address].filter(Boolean).join(" · ") || undefined}
      >
        ...
        {vendor.payment_amount && (
          <div className="text-sm text-muted border-t border-border pt-3">
            Agreed to pay ₹{vendor.payment_amount} every {vendor.payment_frequency === "weekly" ? "week" : "month"}.
            Full hisab is usually reconciled every 6–8 months — the ledger below has everything you need for that.
          </div>
        )}
      </Panel>
...
function RecordEntryPanel({
  ...
  async function submit() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/credit-vendors/${vendorId}/ledger`, {
        method: "POST",
      ...
      const balRes = await fetch(`/api/credit-vendors/${vendorId}`);
